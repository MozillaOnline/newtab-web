...
export const SectionMenuOptions = {
  ...
  MoCoCNLessRows: section => {
    return {
      ...
      icon: "minimize",
      action: (
        section.mococnNumRows &&
        ac.SetPref(section.mococnNumRows.pref, section.mococnNumRows.val - 1)
      ),
      disabled: (
        !section.mococnNumRows ||
        section.mococnNumRows.val <= section.mococnNumRows.min
      ),
  ...
  MoCoCNMoreRows: section => {
    return {
      ...
      disabled: (
        !section.mococnNumRows ||
        section.mococnNumRows.val >= section.mococnNumRows.max
      ),
